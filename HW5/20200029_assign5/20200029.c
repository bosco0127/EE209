...
int main() {
  /* filename */
  char filename[100];
  /* for storing current working directory */
  char dirname[100];
  ...
  void (*pfRet)(int);
  /* signal unblock */
  sigset_t iSet, qSet, aSet, cSet, conSet;

  /* store current working directory */
  getcwd(dirname,sizeof(dirname));

  /* Get file name */
  sprintf(filename,"%s/.ishrc",HOME);

  /* Get file pointer that read .ishrc */
  ...
  sigprocmask(SIG_UNBLOCK, &conSet, NULL);
  ...
  while(fp != NULL) {
    ...
    /* if gets nothing, exit */
    if(fgets(command, sizeof(command), fp)==NULL)
      break;

    /* Print "% " prompt with command */
    printf("%% %s",command);
